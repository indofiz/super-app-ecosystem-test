...
-- Pull the payload out of the JWT. Signature is already verified
-- by the bundled jwt plugin running before us — we only decode.
-- We can't `require "kong.plugins.jwt.jwt_parser"` (sandbox), so
-- do the base64url + JSON dance inline.
--
-- AUDIT S-9: all three captures use `+` (one-or-more) rather than the
-- previous permissive `*` (zero-or-more) on the signature segment. By
-- the time this runs the bundled jwt plugin has already proved the
-- signature is well-formed and valid, so this is belt-and-braces — but
-- it stops a malformed-but-decodable token from sliding through if a
-- future plugin-ordering change ever puts us in front of jwt.
...
-- step 3: re-set headers from verified claims
if claims.sub then
...
-- step 4: strip the bearer before forwarding upstream (AUDIT S-3).
-- Identity is now in X-User-Id / X-Session-Id / X-Roles; the raw
-- JWT has no purpose downstream and only widens the blast radius
-- if an upstream is ever compromised.
